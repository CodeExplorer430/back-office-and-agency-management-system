...
try {
    ...
    ct2Log($ct2Prefix, 'Starting headless Chrome for UI regression validation.');
    ...
    $ct2NodeCommand = escapeshellarg($ct2Node) . ' ' . escapeshellarg(__DIR__ . '/ct2_ui_regression_check.js');
    ...
    if (!is_resource($ct2Process)) {
        ct2Fail($ct2Prefix, 'Unable to start the shared UI regression runner.');
    }
    ...
    if ($ct2ExitCode !== 0) {
        ...
        ct2Fail($ct2Prefix, 'The shared UI regression runner failed.');
    }
} finally {
